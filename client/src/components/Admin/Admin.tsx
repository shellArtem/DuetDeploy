  ...
  useEffect(() => {
    (async () => {
        try {
            ...
            const resp = await fetch('http://localhost:3003/allAnkets', {
                method: 'GET',
                headers: { 'Content-type': 'application/json' },
            });
...
const approveHandler = async (id, approved) => {
  ...
  const onFinishNewExtra = async (values: any) => {
    try {
      const responce = await fetch("http://localhost:3003/extra", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
const deleteEventHandler = async (id) => {
  try {
    const response = await fetch(`http://localhost:3003/allEvents`, {
      method: 'DELETE',
      headers: { 'Content-type': 'application/json' },
      body: JSON.stringify({id}),
    });
...
const exportAnketsToExcelHandler = async () => {
  try {
    const response = await fetch(`http://localhost:3003/exportToExcel`, {
      method: 'GET',
...
const exportWishToExcelHandler = async () => {
  try {
    const response = await fetch(`http://localhost:3003/exportToExcelWish`, {
      method: 'GET',
      headers: { 'Content-type': 'application/vnd.ms-excel' },
    });
    const blob = await response.blob();
  ...
  const onFinish = async (values) => {
    try {
      const responce = await fetch("http://localhost:3003/profile/matches", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
