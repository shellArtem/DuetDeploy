...
export default function OneDate() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await fetch("http://duet-marriage.ru:3003/extra", {
          method: "GET",
        ...
        const resp = await fetch("http://duet-marriage.ru:3003/oneDate", {
          method: "POST",
          headers: { "Content-type": "application/json" },
...
//fetch to  create an order
      const response = await fetch("http://duet-marriage.ru:3003/order", {
        method: "POST",
        headers: { "Content-type": "application/json" },
...
//fetch to disable date
      await response.json();
      const resp = await fetch("http://duet-marriage.ru:3003/disabledDate", {
        method: "POST",
        headers: { "Content-type": "application/json" },
...
//oplata
      const respons = await fetch("http://duet-marriage.ru:3003/pay", {
        method: "POST",
        headers: { "Content-type": "application/json" },
        body: JSON.stringify({ allValues }),
      });
  ...
  const deleteHandler = async (id) => {
    try {
      await fetch(
        `http://duet-marriage.ru:3003/createDate/deleteExtra`,
        {
          method: "DELETE",
          headers: { "Content-type": "application/json" },
          body: JSON.stringify({ id }),
  ...
  return (
    ...
    >
      ...
      <h2 className='text'> <img
            className='img'
              alt="example"
              src={`http://duet-marriage.ru:3003${oneDate.img}`}
              height="400px"
      ...
      >
         <div style={{ display: "flex", flexDirection: "column", textAlign:'left',  fontSize: '20px' }}>
              ...
              >
               <div style={{display: 'flex', gap: '5%'}}><img style={{width: '30px', height: '30px'}} src="http://duet-marriage.ru:3003/icons8-heart-balloon-64.png"></img><h3 style={{fontSize: '20px' }}> романтический декор</h3></div> 
               <div style={{display: 'flex', gap: '5%'}}><img style={{width: '30px', height: '30px'}} src="http://duet-marriage.ru:3003/icons8-table-64.png"></img><h3 style={{fontSize: '20px' }}> стол, стулья</h3></div> 
               <div style={{display: 'flex', gap: '5%'}}><img style={{width: '30px', height: '30px'}} src="http://duet-marriage.ru:3003/icons8-fireplace-64.png"></img><h3 style={{fontSize: '20px' }}>пледы</h3></div>
               <div style={{display: 'flex', gap: '5%'}}><img style={{width: '30px', height: '30px'}} src="http://duet-marriage.ru:3003/icons8-champagne-64.png"></img><h3 style={{fontSize: '20px' }}> шампанское</h3></div>
              <div style={{display: 'flex', gap: '5%'}}><img style={{width: '30px', height: '30px'}} src="http://duet-marriage.ru:3003/icons8-water-64.png"></img><h3 style={{fontSize: '20px' }}>вода</h3></div>
              <div style={{display: 'flex', gap: '5%'}}><img style={{width: '30px', height: '30px'}} src="http://duet-marriage.ru:3003/icons8-raspberry-64.png"></img><h3 style={{fontSize: '20px' }}> фруктовая тарелка</h3></div>
              </div>
            </div>
        <Form
          name="basic"
