...
const deleteHandler = async (id) => {
  ...
  try {
    await fetch("http://duet-marriage.ru:8443/feedback", {
      method: "DELETE",
      headers: {
...
const feedbacks = useSelector((state: RootState) => state.FeedbackReducer.feedbacks);
  ...
  useEffect(() => {
    (async () => {
        try {
            const response = await fetch('http://duet-marriage.ru:8443/feedback', {
                method: 'GET',
            ...
            const resp = await fetch('http://duet-marriage.ru:8443/allAnkets', {
                method: 'GET',
                headers: { 'Content-type': 'application/json' },
            });
...
const approveHandler = async (id, approved) => {
  ...
  const handleDelete = async (id: React.Key) => {
    ...
    try {
      ...
      await fetch("http://duet-marriage.ru:8443/ankets", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const onFinishNewExtra = async (values: any) => {
    try {
      const responce = await fetch("http://duet-marriage.ru:8443/extra", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
const sendNews = React.useCallback(async() => {
  ...
  await axios.put('http://duet-marriage.ru:8443/news', data, {
    headers: {
...
useEffect(() => {
  (async () => {
    try {
      ...
      const resp = await fetch('http://duet-marriage.ru:8443/allEvents', {
        method: "GET",
        headers: { "Content-type": "application/json" },
      })
      const res = await resp.json()
...
const deleteDateHandler = async (id) => {
  try {
    await fetch(`http://duet-marriage.ru:8443/disabledDate`, {
      method: 'DELETE',
      headers: { 'Content-type': 'application/json' },
      body: JSON.stringify({id}),
    });
...
const deleteEventHandler = async (id) => {
  try {
    const response = await fetch(`http://duet-marriage.ru:8443/allEvents`, {
      method: 'DELETE',
      headers: { 'Content-type': 'application/json' },
      body: JSON.stringify({id}),
    });
...
const exportAnketsToExcelHandler = async () => {
  try {
    const response = await fetch(`http://duet-marriage.ru:8443/exportToExcel`, {
      method: 'GET',
...
const exportWishToExcelHandler = async () => {
  try {
    const response = await fetch(`http://duet-marriage.ru:8443/exportToExcelWish`, {
      method: 'GET',
      headers: { 'Content-type': 'application/vnd.ms-excel' },
    });
    const blob = await response.blob();
  ...
  const onFinish = async (values) => {
    try {
      const responce = await fetch("http://duet-marriage.ru:8443/profile/matches", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
<li>наличие автомобиля.</li>
      ...
      <div style={{border:'solid 2px #ff5cb8', width:'100%', borderRadius:'15px', margin:'5px', overflow:'hidden'}}>
        <img src={`http://duet-marriage.ru:8443${el.User.photo}`} style={{width:'400px', height:'300px'}}></img>
